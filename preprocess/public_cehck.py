...
def get_full_s_img(file_path,small_size=16):
    """
    250nm/pxl 自前データは137
    """
    full_img = pyvips.Image.new_from_file(file_path, access='sequential')
    full_img = full_img.numpy()[:,:,:3]
    
    full_img = cv2.resize(full_img, dsize=None, fx= 250/137, fy= 250/137, interpolation=cv2.INTER_LINEAR)
    
    h = full_img.shape[1]//small_size
    w = full_img.shape[0]//small_size

    
    small_img = cv2.resize(full_img,(h,w))
    
    return full_img,small_img
...
patch_size = 2048
file_path =  "/data/RSNA/TGCAliver/17369b97-7b62-42d6-a2a2-3811d6513521/TCGA-DD-AAEA-01Z-00-DX1.FEE3F0DC-297E-405A-913F-899ADFC5B899.svs"
def func(file_path,patch_size=2048,reduce_SIZE=16,DIR="/home/abebe9849/Nploid/debug/debug"):
    uuid = file_path.split("/")[-2]
    tmp= pd.DataFrame()

    #np_img = cv2.imread("/home/abebe9849/Nploid/debug/mini_img.png")
    full_img,small_img = get_full_s_img(file_path,small_size=reduce_SIZE)
    coords = compute_coords(small_img,patch_size=patch_size//reduce_SIZE,precompute=True,min_patch_info=0.35,min_axis_info=0.35,min_consec_axis_info=0.35,min_decimal_keep=0.7)
    ...
    for (v, y, x) in coords:
        ...
        xL0 = x*reduce_SIZE
        xL1 = xL0+patch_size    
        tile_full = full_img[yL0:yL1,xL0:xL1,:]
        if tile_full.shape[0]==0 or tile_full.shape[1]==0:
            continue
        else:
            yL0_s.append(yL0)
            xL0_s.append(yL0)
            ...
            patch_path  = f"{DIR}/{uuid}__{c}.png"
            cv2.imwrite(patch_path,tile_full)
            c+=1
    ...
    tmp["file_path"]=xL1_s
    tmp["uuid"]=uuid
    tmp["full_sys_path"]=file_path
    
    return tmp


svs_all = glob.glob("/data/RSNA/TGCAliver/*/*svs")
...
for s in svs_all[:]:
    print(s)
    tmp = func(s,DIR="/data/RSNA/TGCAliver/MIDTIFF")
    all_df.append(tmp)
    
all_df = pd.concat(all_df).reset_index(drop=True)

all_df.to_csv("/home/abebe9849/Nploid/preprocess/TGCA_test.csv",index=False)

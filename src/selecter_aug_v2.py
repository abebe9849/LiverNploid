...
class TrainDataset(Dataset):
    def __init__(self, df,CFG,train=True,transform1=None):
        ...
        self.CFG = CFG
        self.train = train
        self.labels = self.df['select'].to_numpy()
...
def eval_func(model, valid_loader, device):
    model.to(device) 
    model.eval()
...
def submit(CFG,test):
    ...
    for fold in   range(5):
        ...
        model.load_state_dict(torch.load(f"fold{fold}_{CFG.general.exp_num}_best_AUC.pth", map_location="cpu"))
        models.append(model)
    ...
    for fold in range(5):
        col = f"pred_fold{fold}"
        test[col]=tets_preds[:,fold,:]
    
    
    
    return test

def submit_folds_all(CFG,folds_all):
    torch.cuda.set_device(CFG.general.device)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for fold in  range(5):
        ...
        val_idx = folds_all[folds_all['fold'] == fold].index
        test = folds_all.loc[val_idx].reset_index(drop=True)        
        model = Model_iafoss(base_model=CFG.model.name).to(device)
        model.load_state_dict(torch.load(f"fold{fold}_{CFG.general.exp_num}_best_AUC.pth", map_location="cpu"))
        
        valid_dataset = TrainDataset(test,train=False,transform1=get_transforms(data='valid',CFG=CFG),CFG=CFG)#

        valid_loader = DataLoader(valid_dataset, batch_size=8, shuffle=False, num_workers=12,pin_memory=True)
        tets_preds = eval_func(model, valid_loader, device)
    
        test["pred"]=tets_preds
        
        oof_all = pd.concat([oof_all,test])
        
    
        
    oof_all.to_csv(f"oof_all_{CFG.general.exp_num}.csv",index=False)
...
@hydra.main(config_path=f"/home/abebe9849/Nploid/config/",config_name="base_iafoss__")
def main(CFG : DictConfig) -> None:
    ...
    folds = folds_all
    folds["select"] = np.where(folds["select"].to_numpy()==True,1,0)

    print(folds["select"].value_counts())
    print(folds["WSI_ID"].nunique())
    print(folds.groupby("fold")["select"].mean())
    ...
    test_df = submit(CFG,df_tmp)
    
    test_df.to_csv("test.csv",index=False)
    
    #submit_folds_all(CFG,folds_all)

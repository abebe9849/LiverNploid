...
def inf_func(model, valid_loader, device):
...
def submit(CFG,test,DIR,FOLD):
    ...
    model.load_state_dict(torch.load(f"{DIR}/fold{FOLD}_b0_aug_best_AUC.pth", map_location="cpu"))
    model.head[2]=nn.Identity()
    model.head[3]=nn.Identity()
    model.head[4]=nn.Identity()
    model.head[5]=nn.Identity()
    ...
    test_emb = inf_func(model, valid_loader, device)
...
def submit_folds(CFG,folds,DIR):
    ...
    for i in   range(5):
        model = Model_iafoss(base_model=CFG.model.name).to(device)
        #if i==3:continue #AI049
        model.load_state_dict(torch.load(f"{DIR}/fold{i}_b0_aug_gray_best_AUC.pth", map_location="cpu"))
        
        val_folds = folds[folds['fold'] == i].reset_index(drop=True)
        valid_dataset = TestDataset(val_folds,train=False,transform1=get_transforms(data='valid',CFG=CFG),CFG=CFG)# 
...
def main(CFG : DictConfig) -> None:
    ...
    for FOLD in range(5):
        ...
        fold_emb = submit(CFG,df_tmp,DIR,FOLD)
    
        np.save(f"{DIR}/test_{FOLD}_emb.npy",fold_emb)
    """
    FOLD = 4
    
    df = pd.read_csv("/home/abebe9849/Nploid/src/outputs/SAVE3/dino_unfreeze1_aug/test_wo_duplicate.csv")
        
    fold_emb = submit(CFG,df,DIR,FOLD)

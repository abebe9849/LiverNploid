...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        imgs =  cv2.imread(self.image_paths[idx])[:,:,::-1]#2048,2048,3
        #imgs =  cv2.imread(self.image_paths[idx])#2048,2048,3
...
def submit(CFG,test,DIR):
    ...
    for i in   range(5):
        ...
        model.load_state_dict(torch.load(f"{DIR}/fold{i}_{CFG.general.exp_num}__dino_best_AUC.pth", map_location="cpu"))
        models.append(model)
...
#DIR = "/home/abebe9849/Nploid/src/outputs/2023-02-06/HIPT_unfreeze2_aug_strong_select"
...
def main(CFG : DictConfig) -> None:
    ...
    for csv_path in csvs:
        ...
        test_df.to_csv(DIR+"/TCGA/"+csv_path,index=False)

...
def train_fn(CFG,fold,folds,test_pl=0):
    ...
    if folds.shape[0]!=7453:#select
        ...
        valid_dataset = TrainDataset_(val_folds,train=False,transform1=get_transforms(data='valid',CFG=CFG),CFG=CFG)#
    ...
    freeze = 2
    log.info(f"====unfreeze {freeze} layer===")
    
    if CFG.model.name=="HIPT":
        model = Model_HIPT(base_model="dino_vit_s",freeze=freeze).to(device)
    else:
        model = Model_iafoss(base_model="dino_vit_s",freeze=freeze).to(device)

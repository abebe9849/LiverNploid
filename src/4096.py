...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        large_img = np.empty((256*8,256*8,3))
        for i in range(8):
            for j in range(8):
                index = j+i*8
                
                try:
                    large_img[256*i:256*i+256,256*j:256*j+256,:]=cv2.imread(imgs[index])[:,:,::-1]
                except:
                    print(index)


        image =  self.transform(image=large_img.astype(np.uint8))["image"]
...
@hydra.main(config_path="/home/abebe9849/Nploid/config/",config_name="base_large")

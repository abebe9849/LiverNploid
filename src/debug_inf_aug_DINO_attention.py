...
class Model_HIPT(nn.Module):
    ...
    def get_attention(self, input1):
        shape = input1.size()
        batch_size = shape[0]
        n = shape[1]
        input1 = input1.view(-1,shape[2],shape[3],shape[4])

        attentions = self.model.get_last_selfattention(input1)
...
def inf_func(model, valid_loader, device,DIR,test):
    ...
    tk1 = tqdm(enumerate(valid_loader), total=len(valid_loader))
    w_featmap = 256 // patch_size
    h_featmap = 256 // patch_size
    for i, images in tk1:
        if i<735:continue
        ...
        with torch.no_grad():
            with autocast():
                child = file_names[i]
                save_dir = DIR+f"{child}"
                os.makedirs(save_dir,exist_ok=True)
                attentions = model.get_attention(images).cpu()#.squeeze(0)
                nh = attentions.shape[1] # number of head
                print(attentions.shape)
                images = reverse_normalize(images.detach().to('cpu').squeeze())

                attentions = attentions[:,:, 0, 1:].reshape(64,nh, -1)
                attentions = attentions.reshape(64,nh, w_featmap, h_featmap)

                
                
                attentions = nn.functional.interpolate(attentions, scale_factor=patch_size, mode="nearest")
                attentions = attentions.permute(0,2,3,1).numpy()
                #attentions = np.stack([cv2.applyColorMap(c, cv2.COLORMAP_JET) for c in attentions])[:,:,:,::-1]
                #attentions = patch2mid_tiff(attentions)
                print(images.shape)
                images = patch2mid_tiff(images.numpy().transpose(0,2,3,1))
                plt.figure()
                plt.imshow(images)
                plt.savefig(f"{save_dir}/img.png")
                for j in range(attentions.shape[-1]):
                    atten_head = np.uint8(255 * attentions[:,:,:,j])
                    atten_head = patch2mid_tiff(atten_head)
                    plt.figure()
                    plt.imshow(atten_head)
                    plt.savefig(f"{save_dir}/heat_head{j}.png")
                    

        preds.append(attentions)
...
def submit(CFG,test,DIR,FOLD):
    ...
    model.load_state_dict(torch.load(f"{DIR}/fold{FOLD}_nosiy___HIPT_best_AUC.pth", map_location="cpu"))
    #if i==3:continue #AI049
    model.exam_predictor=nn.Identity()
    ...
    test_emb = inf_func(model, valid_loader, device,DIR,test)
...
def submit_folds(CFG,folds,DIR):
    ...
    for i in   range(5):
        model = Model_HIPT(base_model="dino_vit_s",freeze=1).to(device)
        #if i==3:continue #AI049
        model.load_state_dict(torch.load(f"{DIR}/fold{i}_nosiy___HIPT_best_AUC.pth", map_location="cpu"))
...
def main(CFG : DictConfig) -> None:
    ...
    test_emb = submit(CFG,df,DIR,FOLD)
    #np.save(f"test_{FOLD}_emb.npy",test_emb)
    
    #fold_emb = submit(CFG,val_folds,DIR,FOLD)
    #np.save(f"fold_{FOLD}_emb.npy",fold_emb)

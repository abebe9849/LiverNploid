...
def train_fn(CFG,fold,folds,test_pl=0):
    ...
    all_ = pd.read_csv("/home/abebe9849/Nploid/all_5folds20230309_v2.csv")
    all_ = all_[all_["WSI_ID"].isin(val_folds["WSI_ID"].unique())].reset_index(drop=True)
    assert set(all_["WSI_ID"].unique())==set(val_folds["WSI_ID"].unique()),(all_["WSI_ID"].unique(),val_folds["WSI_ID"].unique())

    tra_folds = folds.loc[trn_idx]
    if type(test_pl)!=type(0):
        tra_folds = pd.concat([tra_folds,test_pl]).reset_index(drop=True)
    
    #if folds.shape[0]!=7453:#select
    train_dataset = TrainDataset_(tra_folds,train=True, transform1=get_transforms(data='train',CFG=CFG),CFG=CFG)#get_transforms(data='train',CFG=CFG)
    valid_dataset = TrainDataset(all_,train=False,transform1=get_transforms(data='valid',CFG=CFG),CFG=CFG)#
    ...
    valid_loader = DataLoader(valid_dataset, batch_size=CFG.train.batch_size*4, shuffle=False, num_workers=12,pin_memory=True)
    ...
    for epoch in range(CFG.train.epochs):
        ...
        all_["pred_tmp"]=preds
        all_g = all_.groupby("WSI_ID")[["pred_tmp","label"]].mean().reset_index()##test戸おなじAUCの計算方法にする


        #each_auc,score =AUC(true=valid_labels,predict=preds)
        AUC_score = roc_auc_score(all_g["label"], all_g["pred_tmp"])
        ...
        all_["pred"]=best_preds

    return best_preds, valid_labels,all_
...
def main(CFG : DictConfig) -> None:
    ...
    CFG.general.exp_num =CFG.general.exp_num +"__"+CFG.model.name
    ...
    oof_g = oof.groupby("WSI_ID")[["pred","label"]].mean().reset_index()##test戸おなじAUCの計算方法にする
    ...
    log.info(f"OOF")
    AUC_score = roc_auc_score(oof_g["label"], oof_g["pred"])
    log.info(f"AUC_score  {AUC_score}")

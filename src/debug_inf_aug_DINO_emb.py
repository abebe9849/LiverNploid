...
def submit(CFG,test,DIR,FOLD):
    ...
    model = Model_HIPT(base_model="dino_vit_s",freeze=1).to(device)
        #if i==3:continue #AI049
    model.load_state_dict(torch.load(f"{DIR}/fold{FOLD}_dino_unfreeze1_aug__dino_best_AUC.pth", map_location="cpu"))
    #if i==3:continue #AI049
...
def submit_folds(CFG,folds,DIR):
    ...
    for i in   range(5):
        model = Model_HIPT(base_model="dino_vit_s",freeze=1).to(device)
        #if i==3:continue #AI049
        model.load_state_dict(torch.load(f"{DIR}/fold{i}_HIPT_unfreeze2_aug__HIPT_best_AUC.pth", map_location="cpu"))
...
def main(CFG : DictConfig) -> None:
    ...
    np.save(f"{DIR}/test_{FOLD}_emb.npy",test_emb)
